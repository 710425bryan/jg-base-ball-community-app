...
create or replace function public.get_quarterly_period_index(p_period_key text)
returns integer
language sql
immutable
...
as $$
  ...
  select case
    when period_key ~ '^[0-9]{4}-Q[1-4]$'
      then substring(period_key from 1 for 4)::integer * 4
        + substring(period_key from 7 for 1)::integer
    else null::integer
  end
  from normalized;
$$;

create or replace function public.get_quarterly_payment_open_period_key(
...
as $$
  with target_date as (
    select case
      when extract(month from coalesce(p_date, (now() at time zone 'Asia/Taipei')::date))::integer in (3, 6, 9, 12)
        and extract(day from coalesce(p_date, (now() at time zone 'Asia/Taipei')::date))::integer >= 25
        then (coalesce(p_date, (now() at time zone 'Asia/Taipei')::date) + interval '1 month')::date
      else coalesce(p_date, (now() at time zone 'Asia/Taipei')::date)
    end as date_value
  )
  select to_char(date_value, 'YYYY') || '-Q' || extract(quarter from date_value)::integer::text
  from target_date;
$$;

create or replace function public.is_quarterly_payment_period_open(
  p_period_key text,
  p_date date default ((now() at time zone 'Asia/Taipei')::date)
)
returns boolean
...
set search_path = public
as $$
  select public.get_quarterly_period_index(p_period_key)
    <= public.get_quarterly_period_index(public.get_quarterly_payment_open_period_key(p_date));
$$;

create or replace function public.guard_profile_payment_submission_quarterly_open_period()
returns trigger
language plpgsql
security definer
set search_path = public
...
declare
  ...
  v_open_period_key text := public.get_quarterly_payment_open_period_key();
begin
  if coalesce(new.billing_mode, '') <> 'quarterly' then
    return new;
  end if;
  ...
  if not public.is_quarterly_payment_period_open(v_period_key) then
    raise exception 'quarterly period % is not open yet; current open period is %',
      v_period_key,
      v_open_period_key;
  end if;
...
create or replace function public.guard_profile_payment_submission_item_quarterly_open_period()
returns trigger
language plpgsql
security definer
set search_path = public
...
drop trigger if exists guard_profile_payment_submission_quarterly_open_period
  on public.profile_payment_submissions;

create trigger guard_profile_payment_submission_quarterly_open_period
before insert or update of billing_mode, period_key
on public.profile_payment_submissions
for each row
execute function public.guard_profile_payment_submission_quarterly_open_period();

drop trigger if exists guard_profile_payment_submission_item_quarterly_open_period
  on public.profile_payment_submission_items;

create trigger guard_profile_payment_submission_item_quarterly_open_period
before insert or update of period_key
on public.profile_payment_submission_items
for each row
execute function public.guard_profile_payment_submission_item_quarterly_open_period();
...
revoke all on function public.get_quarterly_period_index(text) from public;
revoke all on function public.get_quarterly_payment_open_period_key(date) from public;
revoke all on function public.is_quarterly_payment_period_open(text, date) from public;
revoke all on function public.guard_profile_payment_submission_quarterly_open_period() from public;
revoke all on function public.guard_profile_payment_submission_item_quarterly_open_period() from public;
revoke all on function public.get_my_payment_submission_estimate(uuid, text) from public;

grant execute on function public.get_quarterly_period_index(text) to authenticated, service_role;
grant execute on function public.get_quarterly_payment_open_period_key(date) to authenticated, service_role;
grant execute on function public.is_quarterly_payment_period_open(text, date) to authenticated, service_role;
