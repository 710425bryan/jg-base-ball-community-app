...
monthly_context as (
  select
    ...
    coalesce(monthly_fees.total_sessions, stored_monthly_total.total_sessions, 4) as total_sessions,
    coalesce(monthly_fees.deduction_amount, 0) as deduction_amount,
    coalesce(monthly_fees.leave_sessions, leave_stats.leave_sessions, 0) as leave_sessions,
    case
...
select
  ...
  coalesce(
    ...
    greatest(monthly_context.total_sessions - monthly_context.leave_sessions, 0) * monthly_context.per_session_fee - monthly_context.deduction_amount
  ) as amount,
