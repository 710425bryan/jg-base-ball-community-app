...
- `/match-records` 的「未來賽事」可由具 `matches:EDIT` 的使用者手動發送單場賽事通知；「提醒排程」同樣只給 `matches:EDIT` 使用者管理，設定存在 `system_settings.match_reminder_schedule_config`，前端走 `src/services/matchReminderNotifications.ts` 呼叫 RPC 或 `send-match-reminders`，通知 URL 仍使用 `/calendar?match_id=...`。
...
- 月繳付款回報開放期別依 `monthly_fees.calculation_type` / 有效收費模式判斷：校隊計次月費只開放已結束月份，避免本月堂數與請假還沒完整就提前收費；球員固定月繳 `monthly_fixed` 每月 25 日起開放下個月。前端 helper、RPC / DB trigger 與文件規則必須同步。
