...
- `HomeView` 的 Team Members 只統計有效名單：`status in ('退隊', '離隊')` 或 `is_inactive_or_graduated = true` 不顯示、不納入總人數；校隊依 `training_program = 'junior_high_school_team'` 分為國中部，其餘校隊顯示為國小校隊，總人數包含兩類校隊與社區球員但不含教練。
...
- 後台路由 `/registration-forms` 使用 `registration_forms:VIEW`；`CREATE / EDIT / DELETE` 分別控制賽事／範本建立與產檔、metadata 維護及刪除，預設只有 `ADMIN` 擁有四個 action。
...
- 已知版型包含「就是棒臺北」XLSX 30 人、「主委盃 U9」DOCX 20 人、「第二屆眼鏡蛇盃」DOCX 10–14 人，以及指定原檔指紋的「眼鏡蛇盃 U9」PDF 10–14 人；版型偵測、欄位映射與 OOXML / PDF 安全限制集中在 function logic。未知版型、巨集、OLE、外部關聯、路徑穿越、超限 ZIP 或非白名單 PDF 必須拒絕。
