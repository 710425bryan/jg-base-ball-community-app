...
- 全站 `<el-select filterable>` 仍使用 Element Plus 原元件，並由 `src/components/common/AppGlobalSelect.vue` 統一監聽輸入及 `compositionupdate`／`compositionend`，補強手機中文 IME 即使未另送 `input` 事件也能即時篩選；不可在個別手機選單另加第二個搜尋框，也不可繞過共用包裝改成另一套選擇器。
