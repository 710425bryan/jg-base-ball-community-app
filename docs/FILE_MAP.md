...
| `src/components/common/AppGlobalSelect.vue` | 全站 Element Plus Select 包裝，保留原元件 API，從文字事件、聚焦或下拉開啟期間的實際 input value 同步手機中文 IME 單選／多選搜尋 |
...
| `supabase/functions/registration-form-documents/cobraWordLogic.ts` | 第二屆眼鏡蛇盃 Word 結構辨識、10–14 人六欄表格與隊職員填寫；保留版式與其他 ZIP parts |
...
| 檔案 | 用途 |
| --- | --- |
| `src/components/fees/FeeSettings.vue` | 分頁式收費設定：中港校隊計次費率、國中部單次月費／訓練日期 switch、社區逐人計次、社區固定月繳、季費補償與不收費名單 |
| `src/components/fees/FeeSettingMemberEditor.vue` | 計次 / 固定月繳共用成員費率編輯器，手機卡片與桌機表格版型 |
| `src/components/fees/SchoolTeamFees.vue` | 月費管理，以中港總部／國中部分頁呈現各自的搜尋、摘要、CSV 與明細，並包含球員計次月費與社區固定月繳 |
...
| `src/types/schoolTeamMonthlyFee.ts` | 中港校隊／國中部月費計算模式與費率設定型別 |
...
| `supabase/functions/send-match-fee-payment-notifications/index.ts` | 保留的比賽費通知端點，開放流程不再呼叫 |
