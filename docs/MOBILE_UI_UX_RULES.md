...
- 可搜尋的單選／多選維持單一 `el-select filterable` 輸入欄位；全站透過 `AppGlobalSelect` 保留 Element Plus 元件與既有 props／events／slots，並直接同步 `input`、`compositionupdate`、`compositionend`，讓 iOS 中文 IME 即使沒有另外送出 `input` 事件也能更新選項。個別頁面不可再嵌入第二個手機搜尋框。
