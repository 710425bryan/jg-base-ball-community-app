...
- 可搜尋的單選／多選維持單一 `el-select filterable` 輸入欄位；全站透過 `AppGlobalSelect` 保留 Element Plus 元件與既有 props／events／slots，除了同步 `input`、`compositionupdate`、`compositionend`，還會在 Select 聚焦期間觀察實際 input value，讓 iOS 中文 IME 即使沒有送出可用文字事件也能更新選項。觀察只在聚焦時啟動，失焦或卸載即停止；個別頁面不可再嵌入第二個手機搜尋框。
