...
- `EquipmentFormDialog` 的「尺寸 / 序號庫存」可切換「調整排序」，以上下箭頭移動整列尺寸與數量；完成排序只回到欄位編輯，按「儲存」才寫入共用 `equipment.sizes_stock` 陣列順序。取消不寫入，重開依保存順序顯示；重複尺寸仍依首次出現位置合併數量，空白尺寸略過，沿用原有裝備編輯權限與儲存流程，不需額外 migration。
