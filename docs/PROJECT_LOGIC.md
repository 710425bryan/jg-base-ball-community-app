...
- `src/main.ts` 以 `AppGlobalSelect` 包裝全站既有 Element Plus `el-select`；可搜尋的單選／多選會從文字事件同步中文查詢，並在聚焦或 Element Plus `visible-change` 回報下拉開啟期間以 50ms 間隔比對實際 input value，只有值改變才刷新內建或既有自訂篩選。失焦、下拉關閉、輸入節點移除或元件卸載即停止觀察，元件 API、選取值與 options 資料流不變。
...
- 前端精靈包含隊職員資料、有效球員／校隊選取與檢查下載三步；隊職員姓名可從有效教練、管理群、球員或校隊名單搜尋選取並自動帶入 `guardian_phone`，也可自訂姓名與手動修正本次電話；選球員提供所有人、動態 U-level 與清除全選快捷操作，超過版型容量時依背號只選前 N 人並提示；球員預設依背號排序且可調整順序，Excel 上限 30、主委盃 Word 上限 20、眼鏡蛇盃 PDF 與第二屆眼鏡蛇盃 Word 限 10–14 人。
- 球員姓名、肖像授權與照片來源只讀主檔；背號、生日、身分證、投打、學校、年級、守位與備註可依版型作本次輸出 override，但不得回寫名單。守位只在版型有對應欄位時顯示且為非必填，留白不阻擋產檔；左右開弓不可自動轉為 R/L，需人工選擇。眼鏡蛇盃 PDF 與第二屆眼鏡蛇盃 Word 沒有照片格，另要求隊址及每位球員的生日、身分證與年級。
...
- `leave_requests` 新增 / 更新 / 刪除會透過假單同步 trigger 重算所有有日期賽事中 `source = 'leave_request'` 的 `matches.absent_players`，包含補登或修正的歷史假單；手動請假列不會被假單同步刪除。半日假單優先使用 `matches.match_time` 判斷是否與比賽時間重疊；若時間欄位空白，會從 `matches.note` 的「集合時間 / 比賽時間 / 開打時間」等備註抓取時間；仍沒有時間時視為全日。
...
- 假單同步作用於所有有日期賽事，且只比對該場 `players` 出賽名單內的球員；補登、刪除或調整歷史假單時段後，自動請假列與既有比賽費 trigger 都需跟著重算，已付款歷史仍依比賽費 guard 保留。
...
- `src/services/matchFeePaymentNotifications.ts`
...
- 季費已存帳款的金額由 `member_id` 決定歸屬，`member_ids` 是家庭／付款關聯。`private.quarterly_fee_matches_member()` 讓繳費紀錄、付款估算、首頁摘要、管理提醒與補償選取本人同季帳款，僅在沒有本人列時回退舊家庭紀錄；原本 6,700／3,700 元的兄弟帳款不能各自都取 6,700。審核單人或多人付款只更新相同 `member_id` 的列，沒有本人列時新增，不接管另一人的列。migration 僅修改函式，不批次重算帳款、待審／已審付款快照或餘額；既有錯誤回報需另行核對。對應 migration 為 `20260926105948_quarterly_payment_member_ownership.sql`；2026-09-26 唯讀查核已確認正式函式含修正及兩位球員正確金額，但無同名 migration history；此次未執行遠端寫入。
...
- 數量操作統一為「目前可用庫存」：列表、摘要、交易與增減預覽不顯示底層總量；有尺寸時分別填可用件數並自動加總。`EquipmentFormDialog` → `equipmentAvailableStockApi` → `save_equipment_available_stock()` 原子保存 metadata 與庫存，DB 加回淨使用及已核准／備貨且未轉交易的預留量，保留交易／付款歷史。`stock_set` 流水帳保存可用量前後值、原因與操作者。異動版本由交易／請購 trigger 推進，舊表單拒絕覆寫；只改 metadata 時不動庫存。既有尺寸無法對應、超用、總量不一致需先人工核對；不可猜測庫存或刪改交易。需先部署 `20260924014519_equipment_available_stock.sql`。

- `EquipmentFormDialog` 的「各尺寸 / 序號目前可用庫存」可切換「調整排序」，以上下箭頭移動整列尺寸與可用數量；完成排序只回到欄位編輯，按「儲存」才寫入共用陣列順序。取消不寫入，重開依保存順序顯示；重複尺寸依首次出現位置合併數量，空白且數量為零的尺寸略過。單純排序不記為數量異動。
...
- `src/utils/equipmentInventoryAdjustment.ts`
