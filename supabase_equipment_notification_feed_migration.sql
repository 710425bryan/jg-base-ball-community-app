...
create index if not exists equipment_purchase_requests_requested_at_idx
  on public.equipment_purchase_requests (requested_at desc);
...
declare
  ...
  v_can_equipment_review boolean := false;
...
begin
  ...
  v_can_equipment_review := public.has_app_permission('equipment', 'EDIT')
  ...
  equipment_items as (
    select
      ...
      '收到裝備加購申請'::text as title,
      format(
        '%s 送出 %s 項裝備加購申請。',
        coalesce(tm.name, '未知成員'),
        coalesce(request_items.item_count, 0)
      ) as body,
      coalesce(r.requested_at, r.created_at) as created_at,
      format('/fees?tab=equipment&highlight_id=%s', r.id::text) as link,
      r.member_id as highlight_member_id
    from public.equipment_purchase_requests r
    left join public.team_members_safe tm on tm.id = r.member_id
    left join lateral (
      select
