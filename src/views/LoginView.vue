  ...
  <div class="w-full h-full flex items-center justify-center p-4">
    <div class="w-full max-w-sm p-8 md:p-10 bg-white/95 backdrop-blur-md rounded-3xl shadow-[0_20px_60px_rgb(0,0,0,0.08)] border border-white animate-fade-in flex flex-col items-center">
      ...
      <div class="text-center mb-8 w-full">
        <h1 class="text-3xl font-extrabold text-gray-800 mb-2 tracking-tight">棒球社區</h1>
      ...
      <div v-else class="text-center py-4 space-y-4 animate-fade-in w-full">
        <div class="w-16 h-16 bg-green-100 text-green-500 rounded-full mx-auto flex items-center justify-center mb-2 shadow-sm">
        ...
        <h3 class="text-xl font-bold text-gray-800">登入信件已發送！</h3>
        <p class="text-gray-500 font-medium leading-relaxed text-sm">我們已將專屬的登入連結發送至<br/><span class="text-orange-600 font-bold text-base mt-1 inline-block">{{ email }}</span><br/>請至信箱點擊連結完成登入。</p>
        <button @click="isEmailSent = false" class="text-sm font-bold text-gray-400 hover:text-orange-600 transition-colors mt-4">重新輸入信箱</button>
      </div>
    </div>
  </div>
</template>
