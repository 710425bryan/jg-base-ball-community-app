...
create or replace function public.is_monthly_fee_deductible_leave_segment(
...
as $$
  select public.normalize_leave_time_segment(p_leave_time_segment) in ('full_day', 'morning');
$$;
...
training_month_dates as (
  ...
  cross join lateral (
    ...
    from (
      ...
      from unnest(public.get_default_training_month_dates(month_input.month_start)) as default_day(training_date)
      where not exists (
        select 1
        from public.training_month_date_settings settings
        where settings.month_start = month_input.month_start
...
training_month_total as (
  ...
  group by training_month_dates.period_key
),
monthly_context as (
  select
...
revoke all on function public.is_monthly_fee_deductible_leave_segment(text) from public;
revoke all on function public.get_my_payment_submission_estimate(uuid, text) from public;

grant execute on function public.is_monthly_fee_deductible_leave_segment(text) to authenticated, service_role;
