...
- `training_program_settings` 管理 program 名稱、對應 `team_group`、預設星期、時間與場地；執行時不可把週六 / 週日、時間或場地寫死在畫面邏輯。
- `role = 校隊` 不新增身分；國中校隊以 `team_group = 國中校隊` 對應 program，找不到對應時校隊與計次月費成員 fallback 到中港校隊。
