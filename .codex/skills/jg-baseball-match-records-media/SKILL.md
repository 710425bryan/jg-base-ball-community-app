...
description: "Match records, schedule detail, dashboard Next Up, lineup, media, live controller, audio transcription, lineup photo parsing, weather, geocoding, and match reminder workflow for jg-base-ball-community-app. Use when changing /calendar, /match-records, /my-records, dashboard Next Up match behavior, matchesApi, match-records components, MatchFormDialog, MatchDetailDialog, MatchLineupTab, MatchAudioRecorder, MatchLiveController, parse-lineup, transcribe-match-audio, resolve-location, weatherApi, matches-photos storage, match reminders, or match media utilities."
---
...
用這個 skill 處理賽程詳情、Dashboard Next Up、比賽紀錄、陣容、照片、即時比賽控制、語音轉紀錄、陣容照片解析、天氣與地理解析。Google Calendar / iCal 匯入仍優先讀 `jg-baseball-match-calendar-sync`，但匯入後的比賽紀錄與媒體流程讀本 skill。
...
19. 若改 Dashboard Next Up，再讀 `src/views/HomeView.vue`、`src/services/myHome.ts`、`src/composables/useMyHomeNextMatch.ts`、`src/utils/myHomeSnapshot.ts` 與最新 `get_my_home_next_event()` migration
...
- Dashboard Next Up 走 `get_my_home_next_event(p_member_id, p_today)`；只顯示目前選取 linked member 有列在 `matches.players`、台灣今天至六天後且尚未結束的非特訓比賽。空白名單、RPC 錯誤或沒有符合賽事時隱藏卡片，不得 fallback 到未驗證的全隊賽事。
...
- Dashboard Next Up 的 `security definer` RPC 必須保留 `auth.uid()` 與 `profiles.linked_team_member_ids` 驗證，且 `public` / `anon` 不得執行。
