...
- 全站可搜尋單選／多選仍使用 Element Plus `el-select`，由 `src/components/common/AppGlobalSelect.vue` 補強中文 IME 組字期間的即時篩選；不要為手機另加第二個搜尋框或換掉既有元件。修改這層時需測試內建篩選、自訂 `filter-method`、既有 model events 與公開的 `focus`／`blur`。
