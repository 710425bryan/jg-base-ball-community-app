...
- 全站可搜尋單選／多選仍使用 Element Plus `el-select`，由 `src/components/common/AppGlobalSelect.vue` 補強中文 IME 的 `input`、`compositionupdate` 與 `compositionend` 搜尋同步，包含組字更新後未另送 `input` 的手機事件路徑；不要為手機另加第二個搜尋框或換掉既有元件。修改這層時需測試內建篩選、自訂 `filter-method`、既有 model events 與公開的 `focus`／`blur`。
