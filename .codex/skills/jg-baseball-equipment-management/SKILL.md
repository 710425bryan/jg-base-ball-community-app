...
- 請購核准後會建立並連結 `purchase` transaction；`approved` -> `ready_for_pickup` 的庫存 guard 必須先排除目前請購自己的已連結 purchase，再以完整請購數量驗證核准前庫存。其他交易與尚未轉交易的請購預留量不可排除，避免備貨狀態切換重複計算，也不可因此放寬真正的超賣保護。
