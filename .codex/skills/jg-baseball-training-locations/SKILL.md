...
- 場地已配置名單使用 `TrainingLocationVenueMembers` 與 `trainingLocationMemberGroups`，依角色與完整組名分組：校隊先於球員，各自 U 層級由大到小（舊大／中／小組相容），未知組別接在後方；組名含「不參賽」者跨角色統一置底。僅排序呈現，不改寫指派順序或請假／收費語意。
