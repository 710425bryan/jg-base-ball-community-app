...
description: "Training venue, program scope, and player assignment workflow for jg-base-ball-community-app. Use when changing 場地與人員配置、訓練項目分流、/training-locations、training_program_settings、training_venues、training_location_sessions、training_location_assignments、個人首頁本週訓練場地、或 send-training-location-notifications。"
---
...
7. `src/utils/trainingPrograms.ts`
8. `src/utils/trainingLocationNotification.ts`
9. `supabase_training_locations_migration.sql`
10. `supabase_zzzzzzzzzzzzzzzzzz_training_program_scope_migration.sql`
11. 若改連動點名，再讀 `supabase_zzzzzzzzz_training_location_attendance_migration.sql`、`src/views/AttendanceListView.vue`、`src/views/RollCallView.vue`
12. `supabase/functions/send-training-location-notifications/index.ts`
13. 若改首頁呈現，再讀 `src/services/myHome.ts`、`src/types/myHome.ts`、`src/components/home/MyHomeTodayPanel.vue`
...
- 球員池、全隊快捷加入與 roster RPC 列出全部有效球員 / 校隊；program 判斷優先使用 `team_members.training_program`，舊資料才以 `team_group` 對應 `training_program_settings.team_group`，找不到時校隊 / 計次月費 fallback 中港總部 program。`team_group` 是所屬群組（熊隊），不可再拿來鎖住中港 / 新泰身份的群組選單；program 只用來標記球員所屬訓練項目與決定配置主檔 / 預設場地時間 / 通知語意，不限制可編排球員。
