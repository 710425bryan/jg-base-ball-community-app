...
drop function if exists public.list_training_location_roster(date);
  ...
  loop
    ...
    loop
      if not exists (
        ...
        raise exception 'member % is not an active billable player', v_member_id;
      end if;
...
begin
  ...
  if not exists (
    ...
    raise exception 'training location venue requires at least one billable assigned member';
  end if;
...
begin
  ...
  assignment_members as (
    ...
    join public.training_location_assignments a
     ...
     and (
        ...
        or (ae.training_location_session_venue_id is null)
     )
    join public.team_members tm
      on tm.id = a.member_id
    where tm.role in ('球員', '校隊')
  ...
  recorded_members as (
    ...
    join public.attendance_records ar
      on ar.event_id = ae.id
  ),
  member_scope as (
    select member_id from assignment_members
    union
    select member_id from recorded_members
  )
  select
    tm.id,
    tm.name::text,
