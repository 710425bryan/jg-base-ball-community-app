...
- 用途：賽事報名主檔、可重用 XLSX / DOCX / PDF 範本上傳／下載、完整名單選取、OOXML 與白名單 PDF 自動填寫、照片關聯、產檔權限與個資稽核。
...
- 用途：收費、付款回報、球員餘額、比賽費、比賽費開放通知、匯款表單匯入與費用提醒；任何費用相關修改都必須依此 skill 跑完整計算回歸矩陣，不可只驗證單一案例。
