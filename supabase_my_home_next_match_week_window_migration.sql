  ...
  from (
    with event_candidates as (
      select
        ...
        null::text as training_registration_status,
        false as is_training_registration_open
      ...
      where m.match_date between v_today and (v_today + 6)
        and btrim(coalesce(m.match_level, '')) <> '特訓課'
        and exists (
          select 1
          from public.split_match_fee_player_names(m.players) parsed_player
          where public.normalize_match_fee_player_name(parsed_player.player_name)
            = public.normalize_match_fee_player_name(v_member_name)
        )
    )
    select
      id,
